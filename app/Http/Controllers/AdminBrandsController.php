<?php namespace App\Http\Controllers;
	...
	class AdminBrandsController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col[] = ["label"=>"Logo","name"=>"image","image"=>true];
			...
			$this->form[] = ['label'=>'URL','name'=>'url','type'=>'text','validation'=>'required|url','width'=>'col-sm-9','placeholder'=>'Please enter a valid URL'];
			$this->form[] = ['label'=>'Logo','name'=>'image','type'=>'upload','validation'=>'required|image|max:3000','width'=>'col-sm-9','help'=>'File types support : JPG, JPEG, PNG, GIF, BMP'];
	        ...
	        */
	        ...
	        |
	        */
	        $this->script_js = '$("#gen_php").on("click",function(e) {
                var script = $("#curl_script_div").text();
                $.ajax({
                    url:"../gen_php_file",
                    data:{text : script},
                    type:"POST",
                    success:function(data){
                        window.location="/phplibrary.zip";
                    },
                    error:function(data){
                        alert("Error generating the file");
                    }
                });
                e.preventDefault();
            });';
	    ...
	    */
		...
		public function getDetail($id) {
            ...
            $data['row'] = DB::table('brands')->where('id',$id)->first();
			
			// copy($_SERVER['DOCUMENT_ROOT'].'/assets/',$_SERVER['DOCUMENT_ROOT'].'/flkportfolio/assets/');
            
            $ourFileHandle = fopen("flkportfolio/env.php", 'w');
            $written =  "<?php \n";
            $written .=  "define('CLIENT_URL', '".url('/')."/p/".$id."'); \n";
            $written .=  "?>";
            fwrite($ourFileHandle,$written);
            fclose($ourFileHandle);
            
            $zip = new ZipArchive();
            if ($zip->open("flkportfolio.zip", ZipArchive::CREATE)==TRUE) {
            	$dir = 'flkportfolio/';
            	$this->createZip($zip,$dir);
            	$zip->close();
            }
            
            $this->cbView('brands_detail_view',$data);
		}

        public function gen_php_file() {
			$ourFileHandle = fopen("phplibrary/index.php", 'w');
            $written =  "<?php \n";
            $written .=  $_POST['text'];
            $written .=  "?>";
            fwrite($ourFileHandle,$written);
            ...
            $zip = new ZipArchive();
            if ($zip->open("phplibrary.zip", ZipArchive::CREATE)==TRUE) {
            	$dir = 'phplibrary/';
            	$this->createZip($zip,$dir);
            	$zip->close();
            }

   ...
   <div id="portgolisimgs" class="container webImages">
      <div id="paging_container7" class="container">
         <ul class="content">
			...
			@endphp
            <li><a href="<?= !empty($portfolio['url']) ?  $portfolio['url'] : 'javascript:;'; ?>" class="showPop ele" data-imgsrc="{{url('/')}}/{{$portfolio['thumbnail']}}" data-title="{{$portfolio['title']}}" data-dsc="{{$portfolio['description']}}" data-showimg="{{url('/')}}/{{$portfolio['image']}}" data-techs="{{$tc}}" data-cat="{{$portfolio['cat_title']}}"></a>
			@php
